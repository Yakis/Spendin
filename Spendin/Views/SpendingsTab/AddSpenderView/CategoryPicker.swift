...
struct CategoryPicker: View {
    ...
    var body: some View {
        HStack {
            let image = Image(systemName: item.category)
                ...
                LazyVGrid(columns: columns, spacing: 16) {
                    ForEach(categories, id: \.self) { item in
                        Image(systemName: item)
                            .font(.title)
                            .foregroundColor(self.item.category == item ? AdaptColors.theOrange : AdaptColors.adaptText)
                            .onTapGesture {
                                withAnimation {
...
struct SuggestionCategoryPicker: View {
    ...
    var body: some View {
        HStack {
            let image = Image(systemName: suggestion.category)
            DisclosureGroup("Pick a category \(image)", isExpanded: $revealDetails) {
                Spacer()
                LazyVGrid(columns: columns, spacing: 16) {
                    ForEach(categories, id: \.self) { item in
                        Image(systemName: item)
                            .font(.title)
                            .foregroundColor(self.suggestion.category == item ? AdaptColors.theOrange : AdaptColors.adaptText)
                            .onTapGesture {
                                withAnimation {
                                    self.suggestion.category = item
                                    self.revealDetails.toggle()
                                }
                            }
                    }
